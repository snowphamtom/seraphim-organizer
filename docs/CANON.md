...
- **Sync:** `clientOmniGyroSync|dodecaGyroAgreed|clientDodecaSync` — ONE shared hub (`--seal-x/--seal-y` + `syncGyroSharedCenter`); wire+seed in `.omniGyroRig` share cam YPR (`--gyro-yaw/--gyro-pitch/--gyro-roll`) with canvas dodeca — one rigid stack; ONE φ TEMPO (`syncTempoClock` / `--field-T*`); no second dodeca off-hub; no overlay drift
...
- **Depth ladder (back→front):** void → hex-iridescent hero → canvas/procedural dodeca → keyed dodeca wire overlay → keyed seed core → desk (dark glass)
...
- **Live tip:** `?v=<shortsha>` — tip: clientOmniGyroSync|dodecaGyroAgreed (rigid wire+seed+canvas φ-gyro over hex-iridescent)
