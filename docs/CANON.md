...
- **Motion:** one φ family (`--field-T3` / `--Tphi3`); cam.gyro full omni rates (showOffPass punch); reduced-motion → stills / no tumble; idle demo pauses on type/focus
...
- **Live tip:** `?v=<shortsha>` — tip: impressShowpiece|showOffPass|goldMovingNodeGif|clientOmniGyroSync (showpiece depth theater + animated gold nodes + φ omni rigid stack)
